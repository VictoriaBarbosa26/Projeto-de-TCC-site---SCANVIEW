...
if ($mensagem_id > 0 && !empty($resposta)) {
    // Recuperar o ID do administrador logado
    $adm_id = $_SESSION['adm_id'];

    // Inserir a resposta no banco de dados
    $sql = "INSERT INTO respostas (mensagem_id, adm_id, resposta, data_resposta) VALUES (?, ?, ?, NOW())";
    $stmt = $conn->prepare($sql);
    if ($stmt === false) {
        die('Erro ao preparar a consulta: ' . $conn->error);
    }

    $stmt->bind_param("iis", $mensagem_id, $adm_id, $resposta);
    if ($stmt->execute()) {
